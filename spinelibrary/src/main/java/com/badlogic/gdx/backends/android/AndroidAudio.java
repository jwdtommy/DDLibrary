...
public final class AndroidAudio implements Audio {
	...
	public Sound newSound (FileHandle file) {
		...
		if (aHandle.type() == FileType.Internal) {
			try {
				AssetFileDescriptor descriptor = aHandle.getAssetFileDescriptor();
				com.badlogic.gdx.backends.android.AndroidSound sound = new com.badlogic.gdx.backends.android.AndroidSound(soundPool, manager, soundPool.load(descriptor, 1));
				descriptor.close();
				return sound;
			...
			try {
				return new com.badlogic.gdx.backends.android.AndroidSound(soundPool, manager, soundPool.load(aHandle.file().getPath(), 1));
			} catch (Exception ex) {
				throw new GdxRuntimeException("Error loading audio file: " + file, ex);
			}
		}
