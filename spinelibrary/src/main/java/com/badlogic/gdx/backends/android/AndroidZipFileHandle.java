...
public class AndroidZipFileHandle extends AndroidFileHandle {
	...
	public FileHandle child(String name) {
		if (file.getPath().length() == 0)
			return new com.badlogic.gdx.backends.android.AndroidZipFileHandle(new File(name), type);
		return new com.badlogic.gdx.backends.android.AndroidZipFileHandle(new File(file, name), type);
	}
	...
	public FileHandle parent() {
		...
		if (parent == null)
			parent = new File("");
		return new com.badlogic.gdx.backends.android.AndroidZipFileHandle(parent.getPath());
	}
	...
	public FileHandle[] list() {
		...
		for (int i = 0, n = handles.length; i < n; i++)
			handles[i] = new com.badlogic.gdx.backends.android.AndroidZipFileHandle(zipEntries[i].mFileName);
		return handles;
	}
	...
	public FileHandle[] list(FileFilter filter) {
		...
		int count = 0;
		for (int i = 0, n = handles.length; i < n; i++) {
			FileHandle child = new com.badlogic.gdx.backends.android.AndroidZipFileHandle(zipEntries[i].mFileName);
	...
	public FileHandle[] list(FilenameFilter filter) {
		...
		for (int i = 0, n = handles.length; i < n; i++) {
			...
			if (!filter.accept(file, path))
				continue;
			handles[count] = new com.badlogic.gdx.backends.android.AndroidZipFileHandle(path);
			count++;
		}
		if (count < zipEntries.length) {
			FileHandle[] newHandles = new FileHandle[count];
