...
public class AndroidFileHandle extends FileHandle {
	...
	public FileHandle child (String name) {
		name = name.replace('\\', '/');
		if (file.getPath().length() == 0) return new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, new File(name), type);
		return new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, new File(file, name), type);
	}
	...
	public FileHandle parent () {
		...
		return new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, parent, type);
	}
		...
		if (type == FileType.Internal) {
			try {
				...
				for (int i = 0, n = handles.length; i < n; i++)
					handles[i] = new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, new File(file, relativePaths[i]), type);
				return handles;
			} catch (Exception ex) {
		...
		if (type == FileType.Internal) {
			try {
				...
				for (int i = 0, n = handles.length; i < n; i++) {
					...
					FileHandle child = new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, new File(file, path), type);
		...
		if (type == FileType.Internal) {
			try {
				...
				for (int i = 0, n = handles.length; i < n; i++) {
					...
					handles[count] = new com.badlogic.gdx.backends.android.AndroidFileHandle(assets, new File(file, path), type);
					count++;
				}
				if (count < relativePaths.length) {
					FileHandle[] newHandles = new FileHandle[count];
